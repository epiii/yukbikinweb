    ...
    <article id="comment-<?php comment_ID(); ?>">
      <header class="comment-author">
        ...
        <div class="author-meta">
          <?php printf(__('<cite class="fn">%s</cite>', THEME_NAME), get_comment_author_link()) ?>
          <time datetime="<?php echo comment_date('c') ?>"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf(__('%1$s', THEME_NAME), get_comment_date(),  get_comment_time()) ?></a></time>
      ...
      <?php if ($comment->comment_approved == '0') : ?>
            <div class="notice alert-box">
              <p class="bottom"><?php _e('Your comment is awaiting moderation.', THEME_NAME) ?></p>
            </div>
      <?php endif; ?>
...
// Do not delete these lines
  ...
  <section id="comments">
    <div class="notice alert-box">
      <p class="bottom"><?php _e('This post is password protected. Enter the password to view comments.', THEME_NAME); ?></p>
    </div>
  </section>
  <?php
    return;
...
<?php if ( have_comments() ) : ?>
  <section id="comments">
    <h3><?php comments_number(__('No Responses to', THEME_NAME), __('One Response to', THEME_NAME), __('% Responses to', THEME_NAME) ); ?> &#8220;<?php the_title(); ?>&#8221;</h3>
    ...
    <footer>
      <nav id="comments-nav">
        <div class="comments-previous"><?php previous_comments_link( __( '&larr; Older comments', THEME_NAME ) ); ?></div>
        <div class="comments-next"><?php next_comments_link( __( 'Newer comments &rarr;', THEME_NAME ) ); ?></div>
      </nav>
    </footer>
  </section>
<?php else : // this is displayed if there are no comments so far ?>
  ...
  <section id="comments">
    <div class="notice alert-box">
      <p class="bottom"><?php _e('Comments are closed.', THEME_NAME) ?></p>
    </div>
  </section>
  <?php endif; ?>
<?php endif; ?>
...
<section id="respond">
  ...
  <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
    ...
    <p><input name="submit" class="small radius button" type="submit" id="submit" tabindex="5" value="<?php esc_attr_e('Submit Comment', THEME_NAME); ?>"></p>
